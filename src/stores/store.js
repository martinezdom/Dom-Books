...
export const useDataStore = defineStore("data", {
  ...
  getters: {
    cliteralFromCode: (state) => (code) =>
      state.modules.find((module) => module.code == code) || {},
    totalDeLibros() {
    ...
    bookOnCart: (state) => (id) =>
      state.booksOnCart.find((book) => book.id === id),
  },
  actions: {
    async populateBooks() {
      try {
    ...
    async addBook(book) {
      try {
        book.userId = 9;
        const exists = await this.bookByUserAndModule(
          book.userId,
          book.moduleCode
        );
        if (exists) {
    ...
    async editBook(book) {
      try {
        ...
        const index = this.books.findIndex(
          (libro) => libro.id === response.data.id
        );
        if (index !== -1) {
          this.books.splice(index, 1);
        }
    ...
    async addBookToCart(book) {
      try {
        ...
        this.addMessage(
          "Libro con ID " + book.id + " añadido al carrito",
          "success"
        );
      } catch (error) {
        this.addMessage(error, "error");
    ...
    deleteBookFromCart(id) {
      try {
        ...
        if (index !== -1) {
          ...
          this.updateLocalStorage();
          this.addMessage(
            "Libro con ID " + id + " eliminado del carrito",
            "success"
          );
        }
      } catch (error) {
    ...
    async bookByUserAndModule(userId, moduleCode) {
      try {
        const response = await api.books.getBookByUserAndModule(
          userId,
          moduleCode
        );
